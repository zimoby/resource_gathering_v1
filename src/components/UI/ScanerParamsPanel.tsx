  ...
  const memoData = useMemo(() => {
    return [
      ...
      },
      {
        ...
        value: Math.round(activePosition.x) + ":" + Math.round(activePosition.z) + ":" + Math.round(activePosition.y)
      },
    ]
  }, [chunkName, selectedResource, currentLocation.x, currentLocation.y, activePosition.x, activePosition.z, activePosition.y]);

  return (
    <BasicPanelWrapper titleText="Scaner:" opacity={opacity}>
      <div>
        {memoData.map((data, index) => (
          <div className="list-selecting" key={index}
            onClick={() => useGameStore.setState({ message: `Scaner: ${data.title} ${data.value}` })}
          >
            <div>{data.title}</div>
            <div>{data.value}</div>
          </div>
